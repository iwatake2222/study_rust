...
impl<'a> WioSplash<'a> {
    ...
    fn draw_case<D: DrawTarget<Rgb565>>(&self, display: &mut D) -> Result<(), D::Error> {
        egrectangle!(
            ...
            style = primitive_style!(stroke_width = 5, stroke_color = CASE_COLOR, fill_color = BG_COLOR)
        ).draw(display)?;
    ...
    fn draw_buttons<D: DrawTarget<Rgb565>>(&self, display: &mut D) -> Result<(), D::Error> {
        for i in 0..3 {
            egrectangle!(
                top_left = (40 + i * 60, 15),
    ...
    fn draw_image<D: DrawTarget<Rgb565>>(&self, display: &mut D) -> Result<(), D::Error> {
