...
fn main() -> ! {
    ...
    let (mut display, _backlight) = sets.display.init(
        &mut clocks,
        peripherals.SERCOM7,
        &mut peripherals.MCLK,
        &mut sets.port,
    ...
    Text::new("Hello Wio Terminal", Point::new(30, 30)).into_styled(TextStyle::new(Font12x16, Rgb565::GREEN)).draw(&mut display).unwrap();
